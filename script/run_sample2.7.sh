...
if [ ! $1 ]; then
	...
	do
		...
		do
			...
			RES_PATH=sample
			MDL_PATH=sample
			CMD="$GPUSET python2.7 $PROG --train $TRAIN --test $TEST --sgdtype $OPTFUNC --arch $MDL --iter_per_epoch $ITER_EPOCH --out $RES_PATH -m $MAX_ITER --mdl_path $MDL_PATH --record_epoch $NUM_RECORD --dropout True --dropout_ratio $DROPOUT --hidden_size $HDIM --time_length $TLEN --embedding_size $VDIM --input_type embedding"
			echo $CMD
		done
	done
fi
