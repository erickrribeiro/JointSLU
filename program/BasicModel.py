    ...
    def build(self):
        ...
        if not self.default:
            if self.update_f == 'sgd':
                ...
                opt_func = Adagrad(lr=self.learning_rate,
                                   epsilon=self.smooth_eps)
                ...
                opt_func = Adam(lr=self.learning_rate,
                                beta_1=self.beta1,
                                beta_2=self.beta2,
                                epsilon=self.smooth_eps)
            elif self.update_f == 'adamax':
                opt_func = Adamax(lr=self.learning_rate,
                                  beta_1=self.beta1,
                                  beta_2=self.beta2,
                                  epsilon=self.smooth_eps)
            else:
        ...
        if self.arch == 'lstm' or self.arch == 'rnn' or self.arch == 'gru' or self.arch == 'blstm' or self.arch == 'brnn' or self.arch == 'bgru':
            if self.input_type == 'embedding':
                ...
                current = Embedding(input_dim=self.input_vocab_size, output_dim=self.embedding_size,
                                    input_length=self.time_length, mask_zero=True)(raw_current)
            else:
        ...
        # Encode all history and the current utterance first and then start tagging
            ...
            match = concatenate([mem_vec, cur_vec], mode='dot', dot_axes=[2, 1])
            ...
            his_vec = concatenate([mem_vec, match], mode='dot', dot_axes=[1, 1])
            encoder = concatenate([his_vec, cur_vec], mode='sum')
